...
const authorize = (req: Request, res: Response, next: () => void) => {
  ...
  try {
    const payload = jwt.verify(token, jwtSecret) as { tenantId: string; userId: string; type?: string };
...
app.post('/auth/login', loginLimiter, (req: Request, res: Response) => {
  ...
  const token = jwt.sign({ tenantId: tenant.id, userId: user.userId, type: 'access' }, jwtSecret, { expiresIn: tokenExpiration });
  const refreshToken = jwt.sign({ tenantId: tenant.id, userId: user.userId, type: 'refresh' }, jwtSecret, { expiresIn: '7d' });
...
app.post('/auth/refresh', refreshLimiter, (req: Request, res: Response) => {
  ...
  try {
    const payload = jwt.verify(refreshToken, jwtSecret) as { tenantId: string; userId: string; type?: string };
    ...
    const token = jwt.sign({ tenantId: tenant.id, userId: payload.userId, type: 'access' }, jwtSecret, { expiresIn: tokenExpiration });
